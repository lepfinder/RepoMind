...
| 场景 | 注入按钮 | 行为 |
| :--- | :--- | :--- |
| 浏览**未导入**的仓库 | 「导入到 github-index」 | 点击后调用 `POST /api/import`，通过 SSE 流式展示解析→克隆→扫描→同步四步进度。已存在的项目自动识别为「已导入」状态 |
| 浏览**已导入**的仓库 | 「在 github-index 查看」 | 点击后新标签页打开 `http://localhost:3000?project={repo}`，直达项目详情页 |
